...
//           });
  ...
  Plugin.prototype._closeSearch = function(event) {
    ...
    $(".header-search__results-wrapper").hide();
    this.body.off('.trademarkHeaderSearch');

    $('[data-action="open-search"]').removeClass("open");
  ...
  Plugin.prototype._doOpenSearch = function() {
    ...
    setTimeout(function(){ $('[data-action="open-search"]').addClass("open"); }, 500);
  };
  ...
  Plugin.prototype._doSearch = function(event) {
    $(".header-search__results-wrapper").show();
...
//     }
  ...
  Plugin.prototype._doCompleteSearch = function(event) {
    var self = this;
    var input_search = this.headerInput.val().replace("/", " ");
    $.ajax({
      method: 'GET',
  ...
  Plugin.prototype._formSubmitted = function(event) {
    if(this.headerInput.val().trim() != ''){
      ...
      $('.header-search__input').val("");
      this._doOpenSearch();
      $(".header-search__results-wrapper").html("<p class='error'>Search field cannot be empty</p>");
      $(".header-search__results-wrapper").show();
      return false;
    }
  };
...
(function ($) {
  ...
  Plugin.prototype._toggleMiniCart = function(event, addToCartClick) {
    var self = this;

    //close search form
    this.searchInstance = $('.header-search').data('plugin_trademarkHeaderSearch');
    this.searchInstance._closeSearch(event);
    ...
    if($(event.target).parents(".mini-cart-menu").length || addToCartClick == true){
     ...
     this.body.addClass('no-scroll');
      ...
      this.body.on('keyup.trademarkMiniCart', function(outsideEvent) {
        if (outsideEvent.keyCode == 27) {
          self._toggleMiniCart(outsideEvent, false);
        }
      });
  ...
  Plugin.prototype._cartUpdated = function(event, cart, forceOpening, rerender) {
    ...
    if (forceOpening) {
      this._toggleMiniCart(event,true);
    }
...
(function ($) {
  ...
  Plugin.prototype._toggleSidebar = function(event) {
    ...
    if (this.sidebarNav.hasClass('sidebar-nav--open')) {
      ...
      if (undefined !== miniCartInstance && (miniCartInstance.state === 'opening' || miniCartInstance.state === 'opened')) {
        miniCartInstance._toggleMiniCart(event, false);
      }
  ...
  Plugin.prototype._toggleSubMenu = function(event) {
    ...
    sidebarMenu.parent().find("> a").toggleClass("open");
...
(function ($) {
  ...
  Plugin.prototype._init = function(options) {
    ...
    if(options["rv_add_to_card"] == true){
    	this._addToCart();
    }
  ...
  Plugin.prototype._onVariantChanged = function(previousVariant, newVariant) {
    ...
    $("#sony-new-variant").val(newVariant.id);
    $("#sony-new-variant").attr("data-price", newVariant['price']);
  };
  ...
  Plugin.prototype._updateProductPrices = function(previousVariant, newVariant) {
    ...
    if (!newVariant) {
      ...
      if (newVariant['compare_at_price'] > newVariant['price']) {
        var discount = Math.floor((1- ( newVariant['price']/newVariant['compare_at_price']))*100);
       	$(".template-product .product__slideshow .discount").show();
        $(".template-product .product__slideshow .discount .price-save").html(discount+"%");
        this.productPrices.append('<span id="mw-current-variant" class="product__price product__price--new h4" data-id="'+ newVariant['id'] +'" data-price="'+ newVariant['price'] +'" data-money-convertible>' + Shopify.formatMoney(newVariant['price'], window.theme.moneyFormat) + '</span>');
        this.productPrices.append('<span class="product__price product__price--old h4" data-money-convertible>' + Shopify.formatMoney(newVariant['compare_at_price'], window.theme.moneyFormat) + '</span>');
      	if($(".template-product .label.label--on-sale").length){
        	$(".template-product .label.label--on-sale").show();
        }
        else {
        	$('.flex-box.product-box').insertAfter('<div class="product-item__labels labels"><span class="label label--on-sale">On Sale</span> </div>');
        }
      } else {
        $(".template-product .product__slideshow .discount").hide();
        $(".template-product .label.label--on-sale").hide();
        this.productPrices.append('<span id="mw-current-variant" class="product__price h4" data-id="'+ newVariant['id'] +'" data-price="'+ newVariant['price'] +'" data-money-convertible>' + Shopify.formatMoney(newVariant['price'], window.theme.moneyFormat) + '</span>');
      }
  ...
  Plugin.prototype._onSelectorChanged = function() {
    ...
    $(".add-to-wishlist").attr("variantid",this.currentVariant.id);

    var data_vars = $(".template-product .input_variants").attr("data_variants");
    var initialValue = $(".template-product .wishlist_collections").attr("variantid");

      var wishlistarray = data_vars != null ? data_vars.split(",") : "";

    if ($.inArray($(".template-product .wishlist_collections").attr("variantid"), wishlistarray) !== -1) {
      $(".template-product .wishlist_collections").addClass("btn-active");
       $(".template-product .wishlist_collections").attr("title","Remove Wishlist");

    }else{
      $(".template-product .wishlist_collections").removeClass("btn-active");
      $(".template-product .wishlist_collections").attr("title","Add Wishlist");
    }
...
//       var errorMessage = error.responseJSON['description'];
      var errorMessage = window.languages.sold_out_message;
	  var addToCartVariant = self.addToCartButton.find("> span").text();
      self.addToCartButton.text(errorMessage)
        .removeAttr('disabled')
        .removeClass('button--loading');
      self.addToCartButton.attr("old-text", addToCartVariant);

      setTimeout(function() {
//         self.addToCartButton.text(window.languages.productFormAddToCart);
        self.addToCartButton.text(window.languages.productFormAddToCart);
        var attr = self.addToCartButton.attr('old-text');

        if (typeof attr !== typeof undefined && attr !== "") {
          // Element has this attribute
         self.addToCartButton.append("<span>" + attr + "</span>");
        }
...
router.route('collections/*type', function() {
  ...
  (function() {
    ...
    var bindFilters = function() {
      $('.collection-sort-by select').val(window.theme.collectionSortBy).on('change', function () {
...
router.route('', function() {
  ...
  (function() {
    var initFullScreenSlideshow = function(slideshow) {
      if($( window ).width() <= 480){
        const heightBanner = $( window ).width() * 4/3;
        $('.aspect-ratio.aspect-ratio_mb').css('height',heightBanner);
        $('.fs-slideshow__plyr-container .plyr-video').css('height',heightBanner);
      }
      ...
      $(".fs-slideshow__slide").each(function( index ) {
...
//         console.log("end:" + end);
      ...
      slideshow.slick({
        ...
        prevArrow: '<button alt="Prev" title="Prev" class="fs-slideshow__nav-button fs-slideshow__nav-prev slick-prev"><span>Prev</span><svg class="icon icon-arrow-left"><use xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="#icon-arrow-left"></use></svg></button>',
        nextArrow: '<button alt="Next" title="Next" class="fs-slideshow__nav-button fs-slideshow__nav-next slick-next"><span>Next</span><svg class="icon icon-arrow-right"><use xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="#icon-arrow-right"></use></svg></button>'
      });
      ...
      slideshow.on('afterChange', function(event, slick, currentSlide) {
        if (slidesCount.length > 0) {
          $( ".fs-slideshow-page" ).removeClass("fs-slideshow__current-slide");
          $( ".fs-slideshow-page" ).each(function( index ) {
            if($(this).text() == currentSlide + 1){
              $(this).addClass("fs-slideshow__current-slide");
            }
          });
...
(function() {
  var initSliderSection = function(domElement) {
    if (domElement.length === 0) {
      return;
    }
  ...
  initSliderSection($('.shopify-section__slider-section .list-products'));

  $(document).on('shopify:section:unload', '.shopify-section__slider-section', function(event) {
    $(event.target).find('.list-products').slick('unslick');
  });

  $(document).on('shopify:section:load', '.shopify-section__slider-section', function(event) {
    initSliderSection($(event.target).find('.list-products'));
  });
